...
from declarations import (SEQ_LEN, MAX_KEY_LEN)
import string
...
def _decypher(cyphertext, key):
    ...
    print('dasdasd: '+str(len(blocks))+' ads '+str(len(cols)))
    decyphered_blocks = text_preparation.to_blocks([fa.shift(col, shift) for col, shift in zip(cols, shifts)])
    last_decyphered_block = text_preparation.to_blocks([fa.shift(last_col, shift) for last_col, shift in zip(last_col, shifts)])
